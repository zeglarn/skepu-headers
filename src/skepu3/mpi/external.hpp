#pragma once
#ifndef SKEPU_MPI_EXTERNAL_HPP
#define SKEPU_MPI_EXTERNAL_HPP 1
...
-> void
{
	...
	// pack_expand(std::get<CI>(cs).gather_to_root()...);
	pack_expand((skepu::cluster::gather_to_root(std::get<CI>(cs)),0)...);
}
...
-> void
{
	...
	// pack_expand(std::get<CI>(cs).getParent().scatter_from_root()...);
	pack_expand((skepu::cluster::scatter_from_root(std::get<CI>(cs)),0)...);
}
